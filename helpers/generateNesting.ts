...
function getNestingLevels(
  ...
  result.push(level)
  if (list.length !== 0) {
    let nestedLevel = getNestingLevels(list, level.map(el => el.id), parentKey)
...
export default function generateNesting(list: any[], nestingKey = "list", parentKey = "parentId"): any[] {
